...
while True:
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            for id, landmark in enumerate(hand_landmarks.landmark):
                ...
                if id == 8:
                    ...
                    scaledDist = 20 * ((clamped - 0.15)/(1.20 - 0.15)) 

                    clampedIndex = max(min(ptIndexx,1),0)
                    scaledDistIndex = 80 * clampedIndex 
        ...
        if lastSentValue > 1:
            serialInst.write((str(scaledDistIndex)+","+str(scaledDist)+"\n").encode('utf-8'))

        lastSentValue = scaledDist
    ...
    cv2.putText(frame, str(round(scaledDistIndex,2)),(5,25),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2,cv2.LINE_AA)
